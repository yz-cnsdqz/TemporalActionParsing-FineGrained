...
def compact_bilinear_pooling_layer(bottom1, bottom2, output_dim, sum_pool=True,
    ...
    # print('bottom1_flat='+str(bottom1_flat.shape))
    ...
    # print('cbp_flat.shape='+str(cbp_flat.shape) )
    output_shape = tf.add(tf.multiply(tf.shape(bottom1), [1, 1, 0]),
                          [0, 0, output_dim])
    
    # print(tf.shape(bottom1))
    cbp = K.reshape(cbp_flat, [-1, bottom1.get_shape().as_list()[1], output_dim ] )
