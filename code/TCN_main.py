...
# for conv in [5, 10, 15, 20]:
    ...
    for split in data.splits:
        ...
        # --------- CVPR model ----------
            ...
            model.fit(X_train_m, Y_train_, nb_epoch=nb_epoch, batch_size=1,
        ...
        # --------- bilinear model (our contribution) ---------#
        elif model_type in ["ED-Bilinear"]:
            # Go from y_t = {1...C} to one-hot vector (e.g. y_t = [0, 0, 1, 0])
            Y_train = [np_utils.to_categorical(y, n_classes) for y in y_train]
            Y_test = [np_utils.to_categorical(y, n_classes) for y in y_test]
            ...
            # import func_bilinear_trainval as bilinear_trainval
            # split_ = int(split[-1])
            # act_fun = 'norm_relu'
            # bilinear_trainval.train(dataset, X_train, Y_train, 
            #                         n_classes, conv, n_nodes, act_fun,
            #                         2**len(n_nodes)*(conv+1), split_)


            # AP_test, ay_test = bilinear_trainval.test(dataset, X_test, y_test,
            #                         n_classes, conv, n_nodes, act_fun,
            #                         2**len(n_nodes)*(conv+1), split_)
            # P_test = [p.argmax(1) for p in AP_test]
